...
function calculateCtc(v: Record<string, string>): CtcResult | null {
  ...
  const hraExemption = Math.min(monthlyHra, monthlyBasic * rentFraction);
  const annualPf = monthlyPf * 12;
  const annualStdDed = v.regime === 'old' ? 50000 : 75000;
  const annualHraExemption = v.regime === 'old' ? hraExemption * 12 : 0;
  const grossIncome = annualCtc - annualPf;
  const taxableIncome = Math.max(0, grossIncome - annualStdDed - annualHraExemption);
  const annualTax = v.regime === 'old' ? calcOldRegimeTax(taxableIncome) : calcNewRegimeTax(taxableIncome);
  const monthlyTax = annualTax / 12;
  const monthlyInHand = monthlyGross - monthlyPf - profTaxMonthly - monthlyTax;
  return { annualCtc, monthlyBasic, monthlyHra, monthlyPf, monthlyGross, monthlyProfTax: profTaxMonthly, monthlyTax, monthlyInHand, regime: v.regime as 'old' | 'new' };
}

const DEFAULTS = { ctc: '1200000', basicPct: '40', hraPct: '50', city: 'metro', pfPct: '12', profTax: '200', regime: 'new' };

const STEPS: StepDef[] = [
  {
    question: 'Annual CTC',
    description: 'Your Cost to Company — the total salary package offered.',
    ...
    validate: (v) => (!v.ctc || Number(v.ctc) <= 0) ? { ctc: 'Enter a valid CTC' } : {},
    formatReview: (v) => formatINR(parseFloat(v.ctc || '0')),
  },
  {
    question: 'Basic % of CTC',
    description: 'Percentage of CTC that is your basic salary (typically 40–50%).',
    ...
    validate: (v) => {
      if (!v.basicPct || Number(v.basicPct) < 20 || Number(v.basicPct) > 80) return { basicPct: 'Enter 20–80%' };
      return {};
    },
  ...
  },
  {
    ...
    description: 'House Rent Allowance as a percentage of your basic salary.',
  ...
  {
    question: 'City Type',
    description: 'Metro cities get 50% HRA exemption; non-metro gets 40%.',
    fields: ['city'], fieldLabels: ['City Type'],
    renderInput: ({ values: v, onChange }) => (
      <OptionCards<string>
        options={[
          { value: 'metro', label: 'Metro', description: 'Mumbai, Delhi, Kolkata, Chennai — 50% HRA exemption' },
          { value: 'non-metro', label: 'Non-Metro', description: 'All other cities — 40% HRA exemption' },
        ]}
        value={v.city}
  ...
  },
  {
    ...
    description: 'Employee Provident Fund contribution (usually 12%).',
    fields: ['pfPct'], fieldLabels: ['PF %'],
    renderInput: ({ values: v, onChange }) => <StepInput value={v.pfPct} onChange={(val) => onChange('pfPct', val)} suffix="% of basic" placeholder="12" min={0} max={24} autoFocus />,
    validate: () => ({}),
    formatReview: (v) => `${v.pfPct}% of basic`,
  },
  {
    question: 'Professional Tax',
    description: 'Monthly professional tax deducted (usually ₹200).',
    fields: ['profTax'], fieldLabels: ['Professional Tax'],
    renderInput: ({ values: v, onChange }) => <StepInput value={v.profTax} onChange={(val) => onChange('profTax', val)} prefix="₹" suffix="per month" placeholder="200" min={0} autoFocus />,
    validate: () => ({}),
    formatReview: (v) => `₹${v.profTax}/month`,
  },
  {
    question: 'Income Tax Regime',
    description: 'New regime has lower rates; old regime allows deductions.',
    fields: ['regime'], fieldLabels: ['Tax Regime'],
    renderInput: ({ values: v, onChange }) => (
      <OptionCards<string>
        options={[
          { value: 'new', label: 'New Regime', description: 'Lower slab rates, ₹75,000 standard deduction. Best for most salaried employees.' },
          { value: 'old', label: 'Old Regime', description: 'Higher rates but allows 80C, HRA, home loan deductions.' },
        ]}
        value={v.regime}
...
export function CtcInhandCalculatorPage() {
  ...
  const result = panel.hasResult ? calculateCtc(panel.values) : null;
      ...
      <div className="space-y-4">
        ...
        {result ? (
          <div className="space-y-4" style={{ opacity: panel.showResult ? 1 : 0, transform: panel.showResult ? 'translateY(0)' : 'translateY(20px)', transition: 'opacity 250ms ease, transform 250ms ease' }}>
            <div className="bg-white rounded-[8px] shadow-card p-5 space-y-1">
              ...
              <div className="py-4 border-y border-[#f0ede6]">
                ...
                <p className="text-4xl font-bold text-[#437a22]">{formatRs(result.monthlyInHand)}</p>
                <p className="text-xs text-[#7a7974] mt-1">Annual: {formatINR(result.monthlyInHand * 12)}</p>
              </div>
              {[
                { label: 'Monthly Gross', value: formatRs(result.monthlyGross) },
                { label: 'Monthly Basic', value: formatRs(result.monthlyBasic) },
                { label: 'HRA', value: formatRs(result.monthlyHra) },
              ].map((row) => (
                <div key={row.label} className="flex justify-between items-center py-2 border-b border-[#f7f6f2] last:border-0">
                  ...
                  <span className="text-sm font-bold text-[#28251d]">{row.value}</span>
                </div>
              ))}
            </div>
          ...
          <div className="bg-white rounded-[8px] shadow-card p-8 flex flex-col items-center justify-center text-center min-h-[260px] space-y-4">
            ...
            <div><p className="text-sm font-medium text-[#28251d]">Find your actual take-home salary</p><p className="text-xs text-[#7a7974] mt-1">7 quick questions to see your salary breakdown</p></div>
