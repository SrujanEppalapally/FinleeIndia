...
function essentialStatus(p: number): StatusInfo {
  if (p <= 50) return { level: 'good', text: 'Essential spending looks healthy and within control.' };
  if (p <= 65) return { level: 'warn', text: 'Essential spending is manageable but should be watched.' };
  return { level: 'critical', text: 'Essential costs are taking too much of your income. Look for areas to reduce fixed expenses.' };
}
function lifestyleStatus(p: number): StatusInfo {
  if (p <= 15) return { level: 'good', text: 'Lifestyle spending is well balanced.' };
  if (p <= 25) return { level: 'warn', text: 'Lifestyle spending is moderate. Keep an eye on non-essential costs.' };
  return { level: 'critical', text: 'Lifestyle spending is high. Consider cutting discretionary expenses.' };
}
function emiStatus(p: number): StatusInfo {
  if (p <= 25) return { level: 'good', text: 'Debt usage looks comfortable.' };
  if (p <= 40) return { level: 'warn', text: 'EMI burden is getting heavier. Review repayment strategy.' };
  return { level: 'critical', text: 'EMIs are too high relative to income. Reducing debt should be a priority.' };
}
function investmentStatus(p: number): StatusInfo {
  if (p >= 20) return { level: 'good', text: 'Investment allocation is strong. Keep building long-term wealth.' };
  if (p >= 10) return { level: 'warn', text: 'Investment contribution is decent, but there is room to improve.' };
  return { level: 'critical', text: 'Investment allocation is low. Try increasing long-term investing.' };
}
function leftoutStatus(p: number): StatusInfo {
  if (p > 20) return { level: 'good', text: 'Excellent monthly surplus. Consider directing this toward emergency fund or investments.' };
  if (p >= 0) return { level: 'warn', text: 'You are saving something each month, but the buffer is limited.' };
  return { level: 'critical', text: 'You are overspending this month. Expenses exceed income.' };
}
      ...
      <div className="p-5 space-y-3">
        {fields.map((f) => (
          <div key={f.key} className="flex items-center gap-3">
            <label className="text-sm text-[#7a7974] flex-1">{f.label}</label>
            <div className="w-40">
              <Input
                type="number"
                placeholder="0"
...
export function MonthlyBudgetPlannerPage() {
  ...
  const chartData = useMemo(() => {
    const items = [
      ...
      { name: 'Leftout', value: Math.max(totals.leftout, 0), color: CHART_COLORS.leftout },
    ];
  ...
  const summaryRows = useMemo(() => {
    return [
      ...
      { name: 'Leftout', value: totals.leftout, pct: percentages.leftout, status: leftoutStatus(percentages.leftout) },
    ];
  }, [totals, percentages]);
    ...
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
        ...
        <div className="space-y-5">
          ...
          <div className="bg-white rounded-[8px] shadow-card overflow-hidden">
            <div className="flex items-center gap-2 px-5 py-4 border-b border-[#f0ede6]">
              <span className="w-2 h-2 rounded-full bg-[#28251d]" />
              <h3 className="text-sm font-semibold text-[#28251d]">Month-End Summary</h3>
            </div>
            <div className="p-5 space-y-3">
              <div className="flex items-center justify-between py-2 border-b border-[#f0ede6]">
                <span className="text-sm text-[#7a7974]">Total Inflow</span>
                <span className="text-sm font-bold text-[#28251d]">{formatINR(totals.totalInflow)}</span>
              </div>
              <div className="flex items-center justify-between py-2 border-b border-[#f0ede6]">
                <span className="text-sm text-[#7a7974]">Total Outflows</span>
                <span className="text-sm font-bold text-[#28251d]">{formatINR(totals.totalOutflows)}</span>
              </div>
              <div className="flex items-center justify-between py-3">
                <span className="text-sm font-medium text-[#28251d]">Leftout for the Month</span>
                <span className={`text-2xl font-bold ${leftoutNegative ? 'text-[#a12c7b]' : 'text-[#01696f]'}`}>
                  {leftoutNegative ? '-' : ''}{formatINR(totals.leftout)}
                </span>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-[8px] shadow-card p-5">
            ...
            {chartData.length === 0 ? (
              ...
              <>
                ...
                {leftoutNegative && (
                  <p className="text-xs text-[#a12c7b] text-center mt-2 font-medium">
                    Leftout is negative — shown as 0 in chart, but actual value is -{formatINR(totals.leftout)}
                  </p>
                )}
              </>
            )}
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-[#f0ede6]">
                ...
                <th className="text-right text-xs font-semibold text-[#7a7974] uppercase tracking-wide px-5 py-3">Value</th>
                ...
                return (
                  <tr key={row.name} className="border-b border-[#f0ede6] last:border-0">
                    <td className="px-5 py-4">
                      ...
                      <span className="text-xs text-[#7a7974] sm:hidden block">{row.pct}%</span>
                    </td>
                    <td className="px-5 py-4 text-right">
                      <span className={`text-sm font-semibold ${row.name === 'Leftout' && leftoutNegative ? 'text-[#a12c7b]' : 'text-[#28251d]'}`}>
                        {row.name === 'Leftout' && leftoutNegative ? '-' : ''}{formatINR(row.value)}
                      </span>
                    </td>
