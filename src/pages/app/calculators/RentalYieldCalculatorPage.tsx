...
function calcRental(v: Record<string, string>): RentalResult | null {
  ...
  let cumulativeRental = 0, totalRentalIncome = 0;
  ...
  for (let y = 1; y <= years; y++) {
    ...
    chartData.push({ year: y, cumulativeRental: Math.round(cumulativeRental), propertyValue: Math.round(pr * Math.pow(1 + appr, y)) });
  }
...
const STEPS: StepDef[] = [
  {
    ...
    description: 'Total cost of buying the property.',
    ...
    validate: (v) => (!v.price || Number(v.price) <= 0) ? { price: 'Enter property price' } : {},
    formatReview: (v) => formatINR(parseFloat(v.price || '0')),
  },
  {
    question: 'Monthly Rent',
    description: 'Current monthly rental income from the property.',
    fields: ['monthlyRent'], fieldLabels: ['Monthly Rent'],
    renderInput: ({ values: v, onChange, errors }) => <StepInput value={v.monthlyRent} onChange={(val) => onChange('monthlyRent', val)} prefix="₹" suffix="per month" placeholder="25000" min={1} error={errors.monthlyRent} autoFocus />,
  ...
  },
  {
    ...
    description: 'Expected percentage increase in rent each year.',
    fields: ['rentIncrease'], fieldLabels: ['Rent Increase'],
    renderInput: ({ values: v, onChange }) => <StepInput value={v.rentIncrease} onChange={(val) => onChange('rentIncrease', val)} suffix="% per year" placeholder="5" min={0} max={30} autoFocus />,
    validate: () => ({}),
    formatReview: (v) => `${v.rentIncrease}% per year`,
  },
  {
    question: 'Annual Maintenance Cost',
    description: 'Yearly maintenance and repair costs.',
    fields: ['maintenance'], fieldLabels: ['Maintenance'],
    renderInput: ({ values: v, onChange }) => <StepInput value={v.maintenance} onChange={(val) => onChange('maintenance', val)} prefix="₹" suffix="per year" placeholder="30000" min={0} autoFocus />,
    validate: () => ({}),
    formatReview: (v) => `${formatINR(parseFloat(v.maintenance || '0'))}/year`,
  },
  {
    question: 'Annual Property Tax',
    description: 'Yearly property tax payable to the municipality.',
    fields: ['propertyTax'], fieldLabels: ['Property Tax'],
    renderInput: ({ values: v, onChange }) => <StepInput value={v.propertyTax} onChange={(val) => onChange('propertyTax', val)} prefix="₹" suffix="per year" placeholder="12000" min={0} autoFocus />,
    validate: () => ({}),
    formatReview: (v) => `${formatINR(parseFloat(v.propertyTax || '0'))}/year`,
  },
  {
    question: 'Annual Price Appreciation',
    description: 'Expected yearly increase in property value.',
    fields: ['appreciation'], fieldLabels: ['Appreciation'],
    renderInput: ({ values: v, onChange }) => <StepInput value={v.appreciation} onChange={(val) => onChange('appreciation', val)} suffix="% per year" placeholder="6" min={0} max={30} autoFocus />,
    validate: () => ({}),
    formatReview: (v) => `${v.appreciation}% per year`,
  },
  {
    question: 'Holding Period',
    description: 'How many years you plan to hold the property.',
    fields: ['holdingYears'], fieldLabels: ['Holding Period'],
    renderInput: ({ values: v, onChange }) => <StepInput value={v.holdingYears} onChange={(val) => onChange('holdingYears', val)} suffix="years" placeholder="10" min={1} max={30} autoFocus />,
    validate: () => ({}),
    formatReview: (v) => `${v.holdingYears} years`,
  },
  {
    question: 'Income Tax Slab',
    description: 'Your marginal income tax rate — rental income is taxed at this rate.',
    fields: ['taxSlab'], fieldLabels: ['Tax Slab'],
    renderInput: ({ values: v, onChange }) => (
      <OptionCards<string>
        options={[
          { value: '0', label: '0% (No Tax)', description: 'Exempt from income tax' },
          { value: '5', label: '5% Slab', description: 'Income up to ₹7 lakh (new regime)' },
          { value: '20', label: '20% Slab', description: 'Income ₹10–12 lakh' },
          { value: '30', label: '30% Slab', description: 'Income above ₹15 lakh' },
        ]}
        value={v.taxSlab}
        onChange={(val) => onChange('taxSlab', val)}
      />
    ),
    validate: () => ({}),
    formatReview: (v) => `${v.taxSlab}% tax slab`,
  },
];
...
export function RentalYieldCalculatorPage() {
  ...
  const result = panel.hasResult ? calcRental(panel.values) : null;
      ...
      <div className="space-y-4">
        ...
        {result ? (
          ...
          <div className="bg-white rounded-[8px] shadow-card p-8 flex flex-col items-center justify-center text-center min-h-[260px] space-y-4">
            ...
            <div><p className="text-sm font-medium text-[#28251d]">Analyse your rental property returns</p><p className="text-xs text-[#7a7974] mt-1">8 quick questions to see gross and net yield</p></div>
