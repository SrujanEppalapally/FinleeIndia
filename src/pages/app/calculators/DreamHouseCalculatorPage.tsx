...
interface HouseResult { futurePrice: number; downPayment: number; loanAmount: number; monthlyEmi: number; sipNeeded: number; chartData: { year: number; price: number }[]; verdict: 'achievable' | 'stretch' | 'revisit'; }
...
function calc(v: Record<string, string>): HouseResult | null {
  ...
  if (!cp || !apr || !yrs || !dp || !lr || !lt) return null;
  const futurePrice = cp * Math.pow(1 + apr, yrs);
  const downPayment = futurePrice * dp, loanAmount = futurePrice - downPayment;
...
function VerdictBadge({ v }: { v: HouseResult['verdict'] }) {
  const cfg = { achievable: { label: 'Achievable', cls: 'bg-[#437a22]/10 text-[#437a22] border-[#437a22]/20' }, stretch: { label: 'Stretch', cls: 'bg-[#b45309]/10 text-[#b45309] border-[#b45309]/20' }, revisit: { label: 'Revisit Plan', cls: 'bg-[#a12c7b]/10 text-[#a12c7b] border-[#a12c7b]/20' } }[v];
  return <span className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-semibold border ${cfg.cls}`}>{cfg.label}</span>;
}

function ChartTip({ active, payload, label }: { active?: boolean; payload?: Array<{ value: number }>; label?: string | number }) {
  if (!active || !payload?.length) return null;
  return <div className="bg-white border border-[#e9e7e1] rounded-[6px] shadow-card px-3 py-2 text-xs"><p className="font-semibold text-[#28251d] mb-1">{label}</p><p className="text-[#7a7974]">Price: <span className="font-medium text-[#28251d]">{formatINR(payload[0].value)}</span></p></div>;
}
...
const STEPS: StepDef[] = [
  {
    ...
    description: 'Today\'s market price of the property you want to buy.',
    fields: ['currentPrice'], fieldLabels: ['Current Price'],
    ...
    validate: (v) => (!v.currentPrice || Number(v.currentPrice) <= 0) ? { currentPrice: 'Enter the property price' } : {},
    formatReview: (v) => formatINR(parseFloat(v.currentPrice || '0')),
  },
  {
    question: 'Property Appreciation Rate',
    description: 'Annual rate at which property prices are expected to rise.',
    fields: ['appreciation'], fieldLabels: ['Appreciation Rate'],
  ...
  },
  {
    ...
    validate: (v) => (!v.years || Number(v.years) <= 0) ? { years: 'Enter years to purchase' } : {},
    formatReview: (v) => `${v.years} years`,
  },
  {
    question: 'Down Payment Percentage',
    description: 'Percentage of the future price you will pay upfront.',
    fields: ['downPct'], fieldLabels: ['Down Payment %'],
    renderInput: ({ values: v, onChange }) => <StepInput value={v.downPct} onChange={(val) => onChange('downPct', val)} suffix="% of price" placeholder="20" min={5} max={100} autoFocus />,
    validate: () => ({}),
    formatReview: (v) => `${v.downPct}%`,
  },
  {
    question: 'Home Loan Interest Rate',
    description: 'Annual interest rate on your home loan.',
    fields: ['loanRate'], fieldLabels: ['Loan Interest Rate'],
    ...
    validate: () => ({}),
    formatReview: (v) => `${v.loanRate}%`,
  },
  {
    question: 'Loan Tenure',
    description: 'Number of years over which you will repay the home loan.',
    fields: ['loanTenure'], fieldLabels: ['Loan Tenure'],
    renderInput: ({ values: v, onChange }) => <StepInput value={v.loanTenure} onChange={(val) => onChange('loanTenure', val)} suffix="years" placeholder="20" min={5} max={30} autoFocus />,
    validate: () => ({}),
    formatReview: (v) => `${v.loanTenure} years`,
  },
  {
    question: 'Your Current Age',
    description: 'Used to personalise the timeline of your home buying plan.',
    fields: ['currentAge'], fieldLabels: ['Your Age'],
    renderInput: ({ values: v, onChange }) => <StepInput value={v.currentAge} onChange={(val) => onChange('currentAge', val)} suffix="years" placeholder="30" min={18} max={70} autoFocus />,
    validate: () => ({}),
      ...
      <div className="space-y-4">
        ...
        {result ? (
          <div className="space-y-4" style={{ opacity: panel.showResult ? 1 : 0, transform: panel.showResult ? 'translateY(0)' : 'translateY(20px)', transition: 'opacity 250ms ease, transform 250ms ease' }}>
            <div className="bg-white rounded-[8px] shadow-card p-5 space-y-1">
              ...
              {[
                ...
                { label: 'Monthly SIP Needed (for down payment)', value: formatINR(result.sipNeeded), color: 'text-[#437a22]' },
              ].map((row) => (
                ...
                <AreaChart data={result.chartData} margin={{ top: 4, right: 8, bottom: 0, left: 0 }}>
                  <defs>
                    <linearGradient id="houseGrad" x1="0" y1="0" x2="0" y2="1">
                      <stop offset="5%" stopColor="#0e7490" stopOpacity={0.25} /><stop offset="95%" stopColor="#0e7490" stopOpacity={0.03} />
                    </linearGradient>
                  </defs>
          ...
          <div className="bg-white rounded-[8px] shadow-card p-8 flex flex-col items-center justify-center text-center min-h-[260px] space-y-4">
            ...
            <div><p className="text-sm font-medium text-[#28251d]">Plan your dream home purchase</p><p className="text-xs text-[#7a7974] mt-1">7 quick questions to see EMI and savings plan</p></div>
